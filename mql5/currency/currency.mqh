...
string StandardCurrencyList[28] = {
...
bool is_standard_currency(string pair) {
    for (int i = 0; i < 28; ++i) {
        if (pair == StandardCurrencyList[i]) return true;
    }
    return false;
}
...
// Clear Market Watch of pairs not in standard 28.
void clear_nonstandard_pairs() {
    string pair;
    bool pair_cleared = false;
    int num_symbols = SymbolsTotal(true);
    for (int i = 0; i < num_symbols; i++) {
        pair = SymbolName(i, true);
        if (! is_standard_currency(pair)) {
            SymbolSelect(pair, false);
            pair_cleared = true;
            break;
        }
    }
    if (pair_cleared) clear_nonstandard_pairs();
}


// Make sure market watch has the 28 currency pairs and ONLY the 28 currency pairs.
void normalize_market_watch() {
    clear_nonstandard_pairs();
    for (int i = 0; i < 28; i++) {
        SymbolSelect(StandardCurrencyList[i], true);
    }
}
